...
class _TapEffectState extends State<TapEffect> with SingleTickerProviderStateMixin {
  ...
  Future<void> onTapCancel() async {
    if (widget.isClickable) {
      await _onDelayed();
      animationController!.animateTo(1.0, duration: const Duration(milliseconds: 240),curve: Curves.fastOutSlowIn);
    }
    isProgress = false;
  }
    ...
    return GestureDetector(
      ...
      onTapDown: (TapDownDetails details) {
        if (widget.isClickable) {
          ...
          animationController!.animateTo(0.9, duration: const Duration(milliseconds: 120),curve: Curves.fastOutSlowIn);
        }
        isProgress = true;
      },
      onTapUp: (TapUpDetails details) => onTapCancel(),
